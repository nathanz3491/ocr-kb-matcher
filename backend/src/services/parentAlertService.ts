...
import { ParentAlert, ParentAlertType, ParentAlertSeverity } from '../../../shared/types';
...
// ─── Storage helpers ──────────────────────────────────────────────────────────
function getAlertFilePath(parentId: string): string {
  return path.join(DATA_DIR, `parent-alerts-${parentId}.json`);
}

async function readAlertFile(parentId: string): Promise<ParentAlert[]> {
  const filePath = getAlertFilePath(parentId);
  try {
    const raw = await fs.readFile(filePath, 'utf-8');
    return JSON.parse(raw) as ParentAlert[];
  } catch {
    return [];
  }
}

async function writeAlertFile(parentId: string, alerts: ParentAlert[]): Promise<void> {
 ...
 */
async function checkQuizAvoidance(studentId: string, thresholdDays = THRESHOLDS.quizAvoidanceDays): Promise<{ triggered: boolean; details: object }> {
  const quizFile = path.join(DATA_DIR, `quiz-results-${studentId}.json`);
  try {
    const raw = await fs.readFile(quizFile, 'utf-8');
    const results: { completedAt?: string }[] = JSON.parse(raw);
    if (!Array.isArray(results) || results.length === 0) {
      return { triggered: true, details: { reason: 'no_quizzes', message: 'No quizzes taken yet' } };
    }
    const latest = results.reduce<{ completedAt: string }>((max, r) =>
      (r.completedAt && r.completedAt > max.completedAt) ? r as { completedAt: string } : max, { completedAt: '1970-01-01' });
 ...
 */
async function checkInactivity(studentId: string, thresholdDays = THRESHOLDS.inactivityDays): Promise<{ triggered: boolean; details: object }> {
  ...
  try {
    ...
    const reviews: { uploadedAt?: string }[] = JSON.parse(raw);
    for (const r of reviews) {
      if (r.uploadedAt) timestamps.push(new Date(r.uploadedAt));
    }
  ...
  try {
    ...
    const jobs: { uploadedAt?: string }[] = JSON.parse(raw);
    for (const j of jobs) {
      if (j.uploadedAt) timestamps.push(new Date(j.uploadedAt));
    }
  } catch { /* no job file */ }
 ...
 */
async function checkOverdueReviews(studentId: string, thresholdDays = THRESHOLDS.overdueReviewsDays): Promise<{ triggered: boolean; details: object }> {
  const reviewFile = path.join(DATA_DIR, `reviews-${studentId}.json`);
  try {
    const raw = await fs.readFile(reviewFile, 'utf-8');
    const reviews: { nextReviewDate?: string; uploadedAt?: string }[] = JSON.parse(raw);
 ...
 */
async function checkOffTopicUpload(studentId: string): Promise<{ triggered: boolean; details: object }> {
  const offTopicFile = path.join(DATA_DIR, `off-topic-jobs-${studentId}.json`);
  try {
    const raw = await fs.readFile(offTopicFile, 'utf-8');
    const entries: { timestamp?: string; confidence?: number }[] = JSON.parse(raw);
 ...
 */
async function getAlerts(parentId: string, studentId?: string): Promise<ParentAlert[]> {
 ...
 */
async function createAlert(alert: Omit<ParentAlert, 'id'>): Promise<ParentAlert> {
 ...
 */
async function resolveAlert(alertId: string, parentId: string): Promise<void> {
  const alerts = await readAlertFile(parentId);
 ...
 */
async function markAlertRead(alertId: string, parentId: string): Promise<void> {
  const alerts = await readAlertFile(parentId);
  const idx = alerts.findIndex(a => a.id === alertId);
  if (idx === -1) return;
 ...
 */
async function dismissAlert(alertId: string, parentId: string): Promise<void> {
  const alerts = await readAlertFile(parentId);
  const idx = alerts.findIndex(a => a.id === alertId);
  if (idx === -1) return;
...
// ─── Service Export ───────────────────────────────────────────────────────────
export const parentAlertService = {
  // Signal checks
  checkQuizAvoidance,
  checkInactivity,
  checkOverdueReviews,
