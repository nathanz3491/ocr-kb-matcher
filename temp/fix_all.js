...
console.log('fnStart:', fnStart, 'exportIdx:', exportIdx);
...
console.log('Patched! Old len:', c.length, 'New len:', newFile.length);

const { exec } = require('child_process');
exec('pm2 restart backend', (e, out, err) => {
    if (e) { console.error('Restart error:', e.message); return; }
    console.log('Restart:', out.trim());
    setTimeout(() => {
        exec('pm2 describe backend', (e2, out2) => {
            const lines = out2.split('\n');
            const pid = lines.find(l => l.includes('pid')) || '';
