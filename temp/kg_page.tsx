      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          <div className="flex items-center gap-4">
            <Link href={studentId ? '/parent-monitor' : '/'}>
              <Button variant="outline" size="sm" className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm">
                <ArrowLeft className="w-4 h-4 mr-2" />
                {studentId ? 'Back to Parent Monitor' : 'Back'}
              </Button>
            </Link>
            <h1 className={cn(
              'text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white',
            )}>
              {studentId ? 'Student Knowledge Graph' : 'Knowledge Graph'}
            </h1>
          </div>
          {!studentId && (
