...
console.log('vi:', vi, 'ti:', ti, 'ri:', ri);

if (vi < 0 || ti < 0 || ri < 0) { console.log('MISSING MARKER'); process.exit(1); }

const section1 = c.substring(0, ti);
const section2 = c.substring(ti, ri);
const section3 = c.substring(ri);

console.log('section1 len:', section1.length, 'ends with:', JSON.stringify(section1.slice(-80)));
console.log('section2 starts with:', JSON.stringify(section2.slice(0, 80)));
console.log('section3 starts with:', JSON.stringify(section3.slice(0, 80)));
