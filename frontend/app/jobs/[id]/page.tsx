...
export default function JobDetailPage() {
  ...
  const [expandedWrongQuestions, setExpandedWrongQuestions] = useState<Set<string>>(new Set());
  const [selectedAnswers, setSelectedAnswers] = useState<Record<string, number>>({});
  const [revealedAnswers, setRevealedAnswers] = useState<Set<string>>(new Set());

  const toggleWrongQuestion = (questionId: string) => {
    setExpandedWrongQuestions(prev => {
          ...
          <div className="relative mb-6 overflow-hidden rounded-2xl border border-amber-300/40 dark:border-amber-700/50 bg-gradient-to-br from-amber-50/80 to-orange-50/80 dark:from-slate-800/90 dark:to-slate-800/70 p-6 shadow-amber-500/10 hover:shadow-amber-500/20 dark:shadow-slate-900/30 backdrop-blur-md transition-all duration-300">
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-4">
                {job.wrongResults.map((qr) => {
                  const isExpanded = expandedWrongQuestions.has(qr.questionId);

                  return (
                    <div key={qr.questionId} className="relative overflow-hidden rounded-xl border border-amber-200/50 dark:border-amber-800/50 shadow-md shadow-amber-500/5 hover:shadow-amber-500/10 transition-all duration-300">
                      {job.jobType === 'WRONG_MULTIPLE' && (
                        <button
                          onClick={() => toggleWrongQuestion(qr.questionId)}
                          className="w-full flex items-center justify-between p-4 bg-gradient-to-r from-amber-50/70 to-orange-50/70 dark:from-slate-700/60 dark:to-slate-700/50 hover:from-amber-100/70 hover:to-orange-100/70 dark:hover:from-slate-600/60 dark:hover:to-slate-500/50 transition-colors"
                        >
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <span className="flex-shrink-0 flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 to-orange-600 text-white text-xs font-bold shadow-md shadow-amber-500/30">
                              {qr.questionIndex}
                            </span>
                            <span className="text-sm font-medium text-slate-700 dark:text-slate-200 text-left truncate">
                              {qr.questionText || `Question ${qr.questionIndex}`}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                            <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-md shadow-amber-500/20">
                      ...
                      {(job.jobType === 'WRONG_SINGLE' || isExpanded) && (
                        <div className="p-5 bg-white/80 dark:bg-slate-800/60 border-t border-amber-200/30 dark:border-amber-800/30">
                          {qr.questionText && job.jobType === 'WRONG_MULTIPLE' && (
                          ...
                          <div className="mb-5">
                            ...
                            <div className="p-4 rounded-xl bg-gradient-to-br from-amber-50/70 to-orange-50/70 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 border-l-4 border-l-amber-500">
                              <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{qr.explanation}</p>
                            </div>
                          </div>
